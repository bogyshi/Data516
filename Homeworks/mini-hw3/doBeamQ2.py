...
if(debug==0):
    ...
    counts = ( lines
        | "split" >> beam.ParDo(classExample).with_output_types(str)
        | "pair_with_1" >> beam.Map(lambda x:(1,len(x)))
        | "grouping" >> beam.GroupByKey()
        | beam.CombineValues(beam.combiners.MeanCombineFn())

    )
    output = counts | "format" >> beam.Map(lambda x:"Avg number of characters per word: %s"%(x[1]))
    output | "write" >> WriteToText(outFile2)
